...
impl IndexingStrategy for ConstantMemoryStrategy {
    fn build(&self, bam_path: &str) -> Result<FlagIndex> {
        ...
        while current_file_pos < file_size {
            ...
            let (chunk_data, _bytes_read) =
                read_chunk_with_overlap(&mut file, current_file_pos, total_chunk_size, file_size)?;
            ...
            // **DISCOVER BLOCKS**
            let (complete_blocks, bytes_consumed) =
                discover_complete_blocks_in_chunk(&chunk_data, current_file_pos, base_chunk_size)?;

            if complete_blocks.is_empty() {
                current_file_pos += if bytes_consumed == 0 {
                    1
                } else {
                    bytes_consumed
                };
            ...
            let chunk_index =
                process_blocks_streaming_no_accumulation(&mut file, &complete_blocks)?;
            ...
            #[cfg(debug_assertions)]
            println!(
                "   {} blocks → {} records total",
                complete_blocks.len(),
                final_index.total_records()
            );
